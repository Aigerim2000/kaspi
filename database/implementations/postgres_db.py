...
class AccountDatabasePostgres(AccountDatabase):
    ...
    def _save_trs(self, transaction: Transaction) -> None:
        if transaction.id_ is None:
            transaction.id_ = uuid4()

        cur = self.conn.cursor()
        cur.execute("""
                UPDATE transactions SET source_account = %s, target_account = %s, balance_brutto = %s, balance_netto= %s, currency = %s, status = %s WHERE id = %s;
        """, (transaction.source_account, transaction.source_account, transaction.balance_brutto, transaction.balance_netto,
              transaction.currency, transaction.status, str(transaction.id_)))
        ...
        if rows_count == 0:
            cur = self.conn.cursor()
            cur.execute("""
                    INSERT INTO accounts (id,source_account, target_account, balance_brutto,balance_netto,currency, balance, status) VALUES (%s, %s, %s
                    , %s, %s , %s, %s);
                    """, (str(transaction.id_), transaction.source_account, transaction.target_account, transaction.balance_brutto,
                          transaction.balance_netto, transaction.currency, transaction.status))
            self.conn.commit()
